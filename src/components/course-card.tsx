      ...
      <CardHeader className="pb-3">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          {course.isEnrolled ? (
            ...
            <span className="shrink-0 rounded-full border px-3 py-0.5 text-xs font-medium transition-colors 
       ...
       <CardContent className="flex items-end justify-between pt-0">
        <div className="space-y-1 text-xs text-muted-foreground"> {course.isEnrolled && ( <> <p>ชื่อ นศ.: {studentName}</p> <p>โปรแกรม: {studentProgram}</p> {enrolledAt && ( <p>ลงทะเบียนเมื่อ: {enrolledAt}</p> )} </> )} </div>
